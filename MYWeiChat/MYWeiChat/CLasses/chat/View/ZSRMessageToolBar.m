...
@interface ZSRMessageToolBar()<UITextViewDelegate, DXFaceDelegate>
...
@implementation ZSRMessageToolBar
...
- (void)sendFace
{
    ...
    if (chatText.length > 0) {
        if ([self.delegate respondsToSelector:@selector(didSendText:)]) {
            ...
            [self willShowInputTextViewToHeight:[self textViewContentH:self.inputTextView]];;
        }
    }
}
...
{
    [self willShowInputTextViewToHeight:[self textViewContentH:textView]];
}
...
- (void)setupSubviews
{
    ...
    _previousTextViewContentHeight = [self textViewContentH:_inputTextView];
    ...
    [self.recordButton setTitle:@"按下 说话" forState:UIControlStateNormal];
    ...
    if (!self.recordView) {
        self.recordView = [[DXRecordView alloc] initWithFrame:CGRectMake(90, 0, 140, 140)];
    }
...
- (CGFloat)textViewContentH:(UITextView *)textView
{
